/**
 * Draft state for the jam attempt logger. `attempts` / `completed` /
 ...
 * the jam sheet's smaller surface (no comments, no grade voting).
 *
 * Pure and unit-tested: call `jamLogReducer(draft, action)` and
 * assert the return — see jamLogReducer.test.ts.
 */

export interface JamLogDraft {
  attempts: number;
  completed: boolean;
  zone: boolean;
}

export type JamLogDraftAction =
  | { type: "set-attempts"; attempts: number }
  | { type: "mark-complete" }
  | { type: "undo-complete" }
  | { type: "set-zone"; zone: boolean };

export function initJamLogDraft(
