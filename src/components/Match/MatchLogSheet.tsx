...
interface Props {
   ...
   * on each call — `MatchScreen` already does that via its offline-
   * queue wrapper.
   *
 ...
 * deliberate:
 *   • No beta-spray drawer — matches are ephemeral and a 20-player
 *     session has too little signal to spray over.
 *   • No community grade display — match routes carry a fixed grade
 *     set at route-creation time, no voting.
 *   • An extra "Edit route" button in the footer so hosts can fix
 *     a mis-typed description or bump the grade mid-match.
 */
export function MatchLogSheet({
  ...
  // unit-tested in matchLogReducer.test.ts.
  const [draft, dispatch] = useReducer(matchLogReducer, log, initMatchLogDraft);
  ...
  const apply = useCallback(
    (action: MatchLogDraftAction) => {
      dispatch(action);
      return matchLogReducer(draftRef.current, action);
    },
    [],
  );
