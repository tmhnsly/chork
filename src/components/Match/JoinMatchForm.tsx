...
import { lookupMatchByCode } from "@/lib/data/match-queries";
...
export function JoinMatchForm({ initialCode }: Props) {
  ...
  const codeValid = MATCH_CODE_RE.test(normalised);
  ...
  const { data: lookup, error, reload } = useClientResource<JoinMatchLookup>(
    normalised,
    async (key) => {
  ...
  function handleJoin() {
    ...
    startTransition(async () => {
      const result = await joinMatchAction(lookup.set_id);
      if ("error" in result) {
        showToast(result.error, "error");
        return;
      }
      router.push(`/match/${lookup.set_id}`);
    });
  }
