-- ────────────────────────────────────────────────────────────────
-- The Match board counts guests
-- ────────────────────────────────────────────────────────────────
--
-- Migration 095 gave a Match seats that have no account. These are
-- the reads that have to know about them.
--
-- **Scope: Matches only.** The gym leaderboard is for gym members
-- who are signed in, and nothing here touches it — a guest's log
-- carries no `gym_id` and belongs to a climber-owned Set, so
-- `get_leaderboard_set`, `get_leaderboard_all_time` and
-- `user_set_stats` cannot see one by construction. That is verified
-- rather than assumed; see the test alongside this migration.
--
-- The identity these functions group by changes from "the account"
-- to "the seat" (`set_players.id`), because a seat is the only thing
-- both kinds of player have. `user_id` comes back nullable, and the
-- display name resolves from the profile when there is one and from
-- the seat when there isn't.

-- ── 1. One ranking, now over seats ────────────────────────────────
...
as $$
  with agg as (
    ...
    -- An account-backed seat owns its logs by `user_id`; a guest seat
    -- owns them by `player_id`. Matching on the seat rather than the
    -- account is what lets both sit on one board.
...
begin
  ...
  select
    ...
    st.user_id,
    p.username,
    -- A profile's name where there is one, the seat's where there
    -- isn't. Never both — `set_players_identity_ck` guarantees it.
    ...
    -- A guest's always read 0 here: there is no account to own them,
    -- and the host who entered them reads them from `guest_logs` in
    -- the room bundle instead. See CONTEXT.md "Attempt privacy".
...
begin
  ...
  return jsonb_build_object(
    ...
    -- Guest logs, for the host only. The host entered these, edits
    -- them, and is the one person for whom they aren't someone else's
    -- private data — a guest has no account to keep them from.
...
--
-- Both read `match_standings`, whose output shape just changed, so
-- both are redefined against the new column names. Neither gains a
-- guest branch: history is per-ACCOUNT (a guest has none), and the
-- public card shows the whole board including guests, which it now
-- does for free.
